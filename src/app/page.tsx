...
function KnownTokenAddButton({
  ...
  return (
    ...
    >
      ...
      {status === "error" && <span className="text-xs text-red-400">فشل</span>}
    </button>
  );
}
...
function HomeContent() {
  ...
  const [view, setView] = useState<"create" | "history" | "send" | "vendor" | "secure">("create");
      ...
      <div className="h-full flex flex-col min-h-0 relative overflow-hidden">
        ...
        <div className="flex flex-1 flex-col md:flex-row min-h-0 relative z-10">
          ...
          <main className="flex-1 min-w-0 overflow-auto px-4 py-6 sm:px-8 w-full scroll-smooth">
            ...
            {linkTokenValid && view === "create" && (
              <section className="mb-4 rounded-2xl border border-indigo-500/30 bg-indigo-500/10 p-4 shadow-lg shadow-indigo-500/5 w-full glass-panel">
                <p className="mb-1 text-sm font-semibold text-slate-100">استيراد التوكن من الرابط</p>
                <p className="mb-3 text-xs text-slate-400">
                  {linkSymbol} على {linkNetwork?.name ?? `الشبكة ${linkChainId ?? "—"}`}
                </p>
                {linkPrice && (
                  <p className="mb-3 rounded-xl bg-emerald-500/20 px-3 py-2 text-sm font-medium text-emerald-100">
                    السعر المعروض: <strong>${linkPrice}</strong> لكل وحدة (من المرسل)
                  </p>
                )}
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    نسخ عنوان العقد (Trust Wallet: أضف توكن مخصص)
                  </button>
                ...
                <p className="mt-2 text-xs text-slate-500">
                  Trust Wallet: إن لم يضف التوكن تلقائياً، اختر الشبكة الصحيحة ثم إضافة توكن مخصص — الصق عنوان العقد، الرمز {linkSymbol}، الخانات العشرية {linkDecimals}.
                </p>
              </section>
            )}
        ...
        <section className="w-full">
          <div className="mb-4 rounded-xl border-2 border-amber-500/50 bg-amber-500/15 p-3 text-sm text-amber-100">
            <p className="font-bold">أرسلت من التطبيق لمحفظتك ولا تشوف الفلوس؟</p>
            <p className="mt-1">المبلغ موجود في محفظتك على الشبكة. المحفظة لا تعرضه حتى تضيف التوكن: اذهب إلى <strong>سجل النشرات</strong> → اضغط <strong>نسخ للمستلم</strong> على النشر المناسب → افتح محفظتك (MetaMask أو Trust) → إضافة توكن مخصص → الصق عنوان العقد، الرمز (مثل USDT)، الخانات العشرية 18. بعدها يظهر الرصيد.</p>
            <p className="mt-2 text-amber-200/90 text-xs">لو شفت في المستكشف «From: 0x0000...0000» — هذا طبيعي. معناها التوكن مُنشأ (mint) ومرسل لك من العقد، مو من محفظة شخص. الرصيد وصل لعنوانك.</p>
          </div>
          <p className="mb-3 text-sm leading-relaxed text-slate-400">
            اختر المستلم، املأ بيانات التوكن، ثم انشر. كامل العرض يُرسل لعنوان واحد.
          </p>

        <section className="rounded-2xl border border-slate-600/60 bg-slate-800/40 backdrop-blur-sm p-4 sm:p-6 shadow-xl shadow-black/20 w-full">
          <h1 className="mb-1 text-xl font-bold text-slate-100">إنشاء توكن</h1>
          <p className="mb-4 text-sm text-slate-500">نشر توكن ERC-20 أو BEP-20 من المتصفح دون سيرفر.</p>

          {/* ——— الخطوة 1 ——— */}
          <div className="mb-4">
            <h2 className="mb-1 text-sm font-semibold uppercase tracking-wider text-indigo-400">الخطوة 1 — من يستلم كامل العرض؟</h2>
            <p className="mb-3 text-xs text-slate-400">كامل العرض يُرسل لعنوان واحد. <strong className="text-amber-200">اختر «محفظتي» لاستلام التوكنات عندك.</strong></p>
            ...
            <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              <label className={`flex cursor-pointer items-center gap-3 rounded-xl border-2 px-4 py-3 transition-all ${receiverChoice === "myWallet" ? "border-emerald-500 bg-emerald-500/20" : "border-slate-600 bg-slate-800/50 hover:border-slate-500"}`}>
                ...
                <div>
                  <span className="font-medium text-slate-100">محفظتي — أستلم أنا التوكنات هنا ✓</span>
              ...
              {addedContractValid && (
                <label className={`flex cursor-pointer items-center gap-3 rounded-xl border-2 px-4 py-3 transition-all ${receiverChoice === "addedContract" ? "border-red-500/60 bg-red-500/15" : "border-slate-600 bg-slate-800/50 hover:border-slate-500"}`}>
                  ...
                  <span className="font-medium text-slate-100">العقد أدناه — لن أستلم، التوكنات تذهب للعقد</span>
                </label>
              )}
              <label className={`flex cursor-pointer items-center gap-3 rounded-xl border-2 px-4 py-3 transition-all ${receiverChoice === "other" ? "border-red-500/60 bg-red-500/15" : "border-slate-600 bg-slate-800/50 hover:border-slate-500"}`}>
                ...
                <span className="font-medium text-slate-100">عنوان آخر — لن أستلم، التوكنات تذهب لهذا العنوان</span>
              </label>
            </div>

            <div className="space-y-3">
              <label htmlFor="receiver-address-input" className="block text-xs font-medium text-slate-400">اختياري: عنوان العقد أو العنوان المستلم</label>
              <input
              ...
              />
              {receiverChoice === "other" && otherAddress && !otherAddressValid && <p className="text-xs text-red-400">عنوان غير صحيح</p>}
              {receiverChoice !== "other" && addedContractAddress && !addedContractValid && <p className="text-xs text-red-400">عنوان غير صحيح</p>}
            </div>

            {receiverChoice === "myWallet" && !walletAddress && <p className="mt-3 text-xs text-amber-400">اتصل بالمحفظة من الأعلى أولاً.</p>}
            {receiverChoice === "myWallet" && walletAddress && (
              <p className="mt-3 rounded-xl border-2 border-emerald-500/40 bg-emerald-500/15 px-4 py-3 text-sm text-emerald-100">
                <strong>ستستلم أنت التوكنات في محفظتك.</strong> بعد النشر أضف التوكن في المحفظة (زر «أضف التوكن للمحفظة» أو من سجل النشرات) لرؤية الرصيد.
              </p>
            )}
            {receiverChoice !== "myWallet" && receiverValid && (
              <p className="mt-3 rounded-xl border-2 border-red-500/40 bg-red-500/15 px-4 py-3 text-sm text-red-100">
                <strong>لن تستلم أنت.</strong> كامل العرض يذهب للعنوان أعلاه — رصيد محفظتك سيبقى 0. لو تريد استلام التوكنات عندك، اختر «محفظتي — أستلم أنا التوكنات هنا ✓».
              </p>
            )}
            <div className="mt-4">
              <label htmlFor="deploy-label" className="block text-xs font-medium text-slate-400">تسمية اختيارية — لأتذكّر هذا النشر لاحقاً</label>
              <input
                id="deploy-label"
                name="deployLabel"
                type="text"
          ...
          {/* ——— الخطوة 2 ——— */}
          <div className="mb-4">
            <h2 className="mb-1 text-sm font-semibold uppercase tracking-wider text-indigo-400">الخطوة 2 — بيانات التوكن</h2>
            <p className="mb-2 text-xs text-slate-400">قوالب جاهزة أو اختر من كتالوج العملات أو املأ يدوياً.</p>

            {/* عملات حقيقية (عقد معروف) — أضف للمحفظة */}
            {network && (() => {
              ...
              return (
                <div className="mb-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 space-y-2">
                  <span className="block text-xs font-medium text-slate-400">عملات حقيقية (عقد معروف) — أضف للمحفظة</span>
            ...
            <div className="mb-4 rounded-xl border border-slate-600/60 bg-slate-800/30 p-3 space-y-3">
              <span className="block text-xs font-medium text-slate-500">كتالوج العملات</span>
              <div className="flex flex-wrap gap-2">
