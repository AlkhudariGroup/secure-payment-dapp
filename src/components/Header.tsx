  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/5 glass-panel">
      <div className="w-full flex h-16 flex-shrink-0 items-center justify-between px-4 sm:px-6">
        <a
          href="/"
          className="text-xl font-bold tracking-tight text-gold hover:text-white transition-colors drop-shadow-md"
        >
          <span className="mr-2">⚡</span>
          مولد التوكنات الملكي
        </a>
        {address ? (
          ...
          <div className="flex flex-wrap items-center justify-end gap-3">
            {hasMultipleWallets && availableProviders.length > 1 && (
              <select
                ...
                title="اختر المحفظة (MetaMask أو Trust)"
              >
            ...
            <button
              ...
              aria-label="اتصال المحفظة"
            >
              {isConnecting ? "جاري الاتصال…" : "اتصال المحفظة"}
            </button>
          </div>
        )}
      </div>
        ...
        <div className="w-full px-4 sm:px-6 py-3 space-y-1 border-t border-white/5 bg-red-900/10 backdrop-blur-sm">
          ...
          {error.includes("رابط RPC") && (
            <p className="text-xs text-slate-400 mt-1">
              RPC جاهز للنسخ: <code className="text-gold/80 bg-black/30 px-1 rounded">https://eth.llamarpc.com</code>
            </p>
          )}
        </div>
      )}
