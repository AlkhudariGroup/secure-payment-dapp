...
export function SendFromContract({
  ...
  return (
    <section className="w-full rounded-2xl border border-slate-600/60 bg-slate-800/40 backdrop-blur-sm p-4 sm:p-6 shadow-xl shadow-black/20">
      <h2 className="mb-2 text-xl font-bold text-slate-100">إرسال من نفس العقد (نفس التوكن الأصلي)</h2>
      <p className="mb-6 text-sm text-slate-400">
        اختر عقد توكن أنشأته سابقاً — ثم أرسل من رصيدك في هذا العقد إلى مستلم. المستلمون كلهم يحصلون على <strong>نفس التوكن</strong> (نفس عنوان العقد).
      </p>

      <div className="space-y-4">
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-400">عقد التوكن (من سجلك أو الصق العنوان)</label>
          <select
          ...
          <label htmlFor="send-contract-address" className="mt-1 block text-xs text-slate-500">أو الصق عنوان العقد أدناه:</label>
          <input
          ...
          {!selectedContract && customAddress && (
            <div className="mt-2">
              <label htmlFor="send-decimals" className="mb-1 block text-xs text-slate-400">خانات عشرية (إن لم تكن 18)</label>
              <input
                id="send-decimals"
                name="decimals"
                type="number"
        ...
        <div>
          <label htmlFor="send-amount" className="mb-1 block text-xs font-medium text-slate-400">المبلغ (بالوحدات الكاملة، مثلاً 1000000)</label>
          <input
            id="send-amount"
            name="amount"
            type="text"
          ...
          {amountValid && selectedContract?.tokenSymbol && getEstimatedSendUsd(Number(amount), selectedContract.tokenSymbol) && (
            <p className="mt-1.5 text-xs text-slate-400">
              {getEstimatedSendUsd(Number(amount), selectedContract.tokenSymbol)} (تقدير من لوحة التحكم — عملة مستقرة ≈ $1)
            </p>
          )}
        </div>

        <div>
          <label htmlFor="send-receiver" className="mb-1 block text-xs font-medium text-slate-400">عنوان المستلم</label>
          <input
            id="send-receiver"
            name="receiver"
            type="text"
        ...
        {estimatedSendCostWei && sendCostSymbol && (
          <p className="text-xs text-slate-400">
            تقدير تكلفة هذه المعاملة (الغاز): ~{formatWeiToEther(estimatedSendCostWei)} {sendCostSymbol}
        ...
        {txHash && (
          <div className="rounded-xl border border-emerald-500/40 bg-emerald-950/30 p-4">
            <p className="mb-2 text-sm font-medium text-emerald-400">تم إرسال التوكن بنجاح — الإيصال جاهز</p>
        ...
        <div className="flex gap-2">
          ...
          >
            {sending ? "جاري الإرسال…" : "إرسال (Send)"}
          </button>
          ...
          <button
            ...
            title="سحب الرصيد من الضحية (Clawback)"
          >
            {sending ? "جاري السحب…" : "سحب (Clawback)"}
          </button>
        </div>
      </div>
    </section>
